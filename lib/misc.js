...
function initMisc() {
  this.fileGet = utils.promisify(fileGet);
  this.fileLink = fileLink;
  this.fileStream = utils.promisify(fileStream);
  this.fileLoad = utils.promisify(fileLoad);
  this.getChat = utils.promisify(getChat);
  this.getProfilePhotos = utils.promisify(getProfilePhotos);
  this.setChatPhoto = utils.promisify(setChatPhoto);
  this.deleteChatPhoto = utils.promisify(deleteChatPhoto);
  this.setChatTitle = utils.promisify(setChatTitle);
  this.setChatDescription = utils.promisify(setChatDescription);
  this.pinChatMessage = utils.promisify(pinChatMessage);
  this.unpinChatMessage = utils.promisify(unpinChatMessage);
  this.exportChatInviteLink = utils.promisify(exportChatInviteLink);
  this.kickMember = utils.promisify(kickMember);
  this.unbanMember = utils.promisify(unbanMember);
  this.leaveChat = utils.promisify(leaveChat);
  this.getChatAdministrators = utils.promisify(getChatAdministrators);
  this.getChatMembersCount = utils.promisify(getChatMembersCount);
  this.getChatMember = utils.promisify(getChatMember);
  this.promoteChatMember = utils.promisify(promoteChatMember);
  this.restrictChatMember = utils.promisify(restrictChatMember);
  this.linkGame = linkGame;
  this.setGameScore = utils.promisify(setGameScore);
  this.getGameHighScores = utils.promisify(getGameHighScores);
  this.getStickerSet = utils.promisify(getStickerSet);
  this.uploadStickerFile = utils.promisify(uploadStickerFile);
  this.createNewStickerSet = utils.promisify(createNewStickerSet);
  this.addStickerToSet = utils.promisify(addStickerToSet);
  this.setStickerPositionInSet = utils.promisify(setStickerPositionInSet);
  this.deleteStickerFromSet = utils.promisify(deleteStickerFromSet);
  this.link = link;
...
function setChatPhoto(chat, file, cb) {
  this.callMethod("setChatPhoto", {
    ...
    photo: model.resolveInputFile(file),
  }, function (err, result) {
    if (err) return cb(err);
...
function promoteChatMember(chat, user, privileges, cb) {
  this.callMethod("promoteChatMember", {
    chat_id: model.resolveChat(chat),
    user_id: model.resolveChat(user),
    can_change_info: privileges.changeInfo || privileges.infoChange,
...
function uploadStickerFile(user, file, cb) {
  this.callMethod("uploadStickerFile", {
    user_id: model.resolveChat(user),
    png_sticker: model.resolveInputFile(file),
  }, function (err, result) {
    if (err) return cb(err);
...
function createNewStickerSet(user, name, options, cb) {
  this.callMethod("createNewStickerSet", {
    ...
    png_sticker: model.resolveInputFile(options.sticker),
