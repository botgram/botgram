// Message facility. This handles incoming messages, calling
...
function processMessage(message) {
  //TODO: get reply queue
  var reply = null;
  message.queued = !this.firstCycleDone;
  callHandler.call(this.handlerQueue, this, 0, message, reply, function () {});
};
...
HandlerQueue.prototype.all = wrapArgs(function (args) {
  ...
  return function all(msg) { return true; };
});
...
HandlerQueue.prototype.mention = wrapArgs(function (args) {
  ...
  if (typeof args[0] !== "boolean") alsoCommands = args.shift();
...
HandlerQueue.prototype.command = wrapArgs(function (args) {
  ...
  return function command(msg) {
    ...
    for (var n = 0; n < args.length; n++)
      if (args[n] instanceof RegExp) return args[n].test(msg.command);
      else return args[n].equalsIgnoreCase(msg.command);
  };
});
...
// FIXME: enable extra filtering (on subject / action)
HandlerQueue.prototype.update = wrapArgs(function (args) {
  if (args.length) throw new Error("Unexpected arguments");
  return function (msg) { return msg.type === "update"; };
});
