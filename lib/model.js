...
ChosenInlineResult.prototype.parse = function parse(choice, options) {
  ...
  if (choice.inline_message_id !== undefined) {
    this.inlineMessage = checkStr(choice.inline_message_id); //FIXME: should be integer???
...
Location.prototype.parse = function parse(location, options) {
  ...
  this.longitude = checkNumber(location.longitude);
  delete location.longitude;

  this.latitude = checkNumber(location.latitude);
  delete location.latitude;
