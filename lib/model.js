...
var messageTypes = {
  ...
  "venue": function (msg, options) {
    ...
    if (options.strict && Object.keys(venue).length)
      throw new Error("unknown fields in venue");
    return true;
  },
  ...
  "update": function (msg, options) {
    if (msg.new_chat_member !== undefined) {
      ...
      this.member = new Chat().parseUser(msg.new_chat_member, options);
    ...
    if (msg.pinned_message !== undefined) {
      this.subject = "message", this.action = "pinned";
...
MessageEntity.prototype.parse = function parse(entity, options) {
  ...
  if (options.strict && Object.keys(entity).length)
    throw new Error("unknown fields in message entity");
  return this;
}
...
InlineQuery.prototype.parse = function parse(query, options) {
  ...
  if (options.strict && Object.keys(query).length)
    throw new Error("unknown fields in inline query");
  return this;
};
...
ChosenInlineResult.prototype.parse = function parse(choice, options) {
  ...
  if (choice.inline_message_id !== undefined) {
    this.inlineMessageId = checkStr(choice.inline_message_id); //FIXME: should be integer???
  ...
  if (options.strict && Object.keys(choice).length)
    throw new Error("unknown fields in chosen inline result");
  return this;
};
...
function parseCommand(msg) {
  //TODO: could we use entities to parse the command as well? Not clear how we would extract username...
  var text = msg.text;
