...
function parseMentions(msg) {
  ...
  while ((i = text.indexOf("@")) !== -1) {
    if (i == 0 || isPunctOrWhitespace.test(text.charAt(i-1))) {
      ...
      var username = text.split(isPunctOrWhitespace)[0];
      if (isValidUsername.test(username)) mentions.push(username);
    } else text = text.substring(i+1);
  }

  msg.mentions = function mentions(x) {
    if (!x) return this.raw;
    var n = 0;
    this.raw.forEach(function (username) {
      if (utils.equalsIgnoreCase(username, x)) n++;
    });
    return n;
  };
  msg.mentions.raw = mentions;
}
...
function parseCommand(msg) {
  ...
  var args = match[5] || "";
  msg.args = function args(x) {
    if (!x) return this.raw;
    var str = this.raw, ret = [], idx;
